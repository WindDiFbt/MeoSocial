...
public interface CommentRepository extends JpaRepository<Comment, String> {
    ...
    List<Comment> findCommentsExistByUserIdOrderByCreatedAtDesc(@Param("userId") Long userId);

    @Query("SELECT c FROM Comment c WHERE c.post.id = :postId ORDER BY c.createdAt DESC ")
    List<Comment> findParentsCommentsExistByPostIdOrderByCreatedAtDesc(@Param("postId") String postId);
